...
class Team implements Comparable<Team> {
    ...
    public Team(int teamNumber, int problems) {
        this.teamNumber = teamNumber;
        solvedProblems = 0;
        totalTime = 0;
        incorrectSubmissions = new int[problems + 1];
        solved = new boolean[problems + 1];
    }

    public void addSubmission(int problem, int time, int judgment) {
        if (!solved[problem]) {
            if (judgment == 0) {
                solved[problem] = true;
                solvedProblems++;
                totalTime += time + (incorrectSubmissions[problem] * 20);
            } else {
                incorrectSubmissions[problem]++;
            }
        }
    }
    ...
    public int compareTo(Team other) {
        if (this.solvedProblems != other.solvedProblems) {
            ...
            return other.teamNumber - this.teamNumber;
        }
    }
}
